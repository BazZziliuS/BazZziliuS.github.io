(function () {
    ...
    class PluginManager {
        ...
        // Блок рекламы
        createAdsBlock() {
            return `<div style="padding: 0.3em 0.3em; padding-top: 0;">
        ...
        setupComponents() {
            ...
            // Подкомпоненты
            this.addSubComponent('add_interface_plugin', this.icons.interface, 'Interface');
        ...
        addSubComponent(component, icon, name) {
            Lampa.SettingsApi.addParam({
                component: 'add_plugin',
        ...
        addPluginParam(component, { name, url, title, author, description }) {
            Lampa.SettingsApi.addParam({
                ...
                onChange: (value) => {
                    ...
                    if (value === '2') this.removePlugin(url);
                },
                onRender: (item) => this.renderPluginStatus(item, url, name)
            });
        }

        // Установка плагина
        installPlugin(url, name, author, itemName) {
            if ($(`DIV[data-name="${itemName}"] .settings-param__status`).hasClass('active')) {
        ...
        removePlugin(url) {
            const plugins = Lampa.Storage.get('plugins');
            const updatedPlugins = plugins.filter(plugin => plugin.url !== url);
        ...
        // Рендеринг статуса плагина
        renderPluginStatus(item, url, name) {
            $('.settings-param__name', item).css('color', '#f3d900');
            this.hideInstallValues();

            setTimeout(() => {
                const status = $('<div class="settings-param__status one"></div>');
                $(`div[data-name="${name}"]`).append(status);
                status.toggleClass('active', this.isPluginInstalled(url));
                status.toggleClass('error', !this.isPluginInstalled(url));
            }, 100);
        }

        // Проверка установки плагина
        isPluginInstalled(url) {
            const plugins = Lampa.Storage.get('plugins') || [];
            return plugins.some(plugin => plugin.url === url);
        }

        // Скрытие значений установок
        hideInstallValues() {
            $("#hideInstall").remove();
        ...
        watchSettings() {
            ...
            const interval = setInterval(() => {
                if (!$('#app > div.settings > div.settings__content.layer--height > div.settings__body > div').length) {
                    ...
                    this.showReloadModal('Для полного удаления плагина перезагрузите приложение!');
                }
            }, 1000);
        }
        ...
        setupListeners() {
            Lampa.Settings.listener.follow('open', (e) => {
                if (e.name === 'main') {
                    setTimeout(() => {
                        $('div[data-component^="add_"]').remove();
                ...
                if (e.name === 'add_plugin') {
                    setTimeout(() => {
                        const statusSpan = $('div > span > div > span');
                        if (statusSpan.text() === 'реклама') {
    ...
    window.appready ? startPlugin() : Lampa.Listener.follow('app', (e) => {
